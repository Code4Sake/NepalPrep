...
import { ceeTopics, ioeTopics, loadAllQuestions } from '../data/questions'
import Navbar from '../components/Navbar'
import styles from './MockExam.module.css'

const EXAM_DURATION = 120 * 60

// Collect all non-comingSoon topic IDs (including subtopics)
function getActiveTopicIds(examType) {
  const topics = examType === 'cee' ? ceeTopics : ioeTopics
  const ids = []
  for (const t of topics) {
    if (t.comingSoon) continue
    if (t.hasSubtopics && t.subtopics) {
      for (const sub of t.subtopics) ids.push(sub.id)
    } else {
      ids.push(t.id)
    }
  }
...
async function buildMockQuestions(examType) {
  const topicIds = getActiveTopicIds(examType)
  const questionsMap = await loadAllQuestions(topicIds)
  const all = []
  Object.entries(questionsMap).forEach(([topicId, qs]) => {
...
export default function MockExam() {
  ...
  const startExam = async () => {
    setLoading(true)
    const mockQs = await buildMockQuestions(exam)
    setQs(mockQs)
