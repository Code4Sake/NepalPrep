...
export default function Tracker() {
  ...
  const [filter, setFilter] = useState('All') // All | CEE | IOE | Backlog
  ...
  // Backlog = has deadline set, deadline passed, not done
  ...
  return (
    <div className={styles.root}>
      ...
      <div className={styles.container}>
        ...
        {/* Exam countdown + overall progress */}
        <div className={styles.topCards}>
          ...
          <div className={styles.countdownCard}>
            {daysLeft !== null ? (
              <>
                ...
                <button className={styles.changeDateBtn} onClick={() => setEditingDate(true)}>
                  Change date
                </button>
              </>
            ) : (
              <>
                ...
                <button className={styles.setDateBtn} onClick={() => setEditingDate(true)}>
                  Set Date →
                </button>
              </>
            )}
          ...
          {backlogTopics.length > 0 && (
            <div className={styles.backlogCard}>
              ...
              <button className={styles.viewBacklogBtn} onClick={() => setFilter('Backlog')}>
                View backlogs →
              </button>
            </div>
          )}
        ...
        <div className={styles.filterTabs}>
          {['All', 'CEE', 'IOE', 'Backlog'].map((f) => (
            <button
              ...
              onClick={() => setFilter(f)}
            >
              {f === 'Backlog' && backlogTopics.length > 0
        ...
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            <thead>
              <tr>
                <th className={styles.thTopic}>Chapter</th>
                <th className={styles.thExam}>Exam</th>
                {COLS.map((c) => (
                  <th key={c} className={styles.thCheck}>{c}</th>
                ))}
                <th className={styles.thDeadline}>Deadline</th>
                <th className={styles.thDays}>Days left</th>
              </tr>
            </thead>
            <tbody>
              {displayTopics.map((topic) => {
                ...
                return (
                  <tr
                    key={topic.id}
                    className={`${styles.row} ${isDone ? styles.rowDone : ''} ${isOverdue ? styles.rowOverdue : ''}`}
                  >
                    <td className={styles.tdTopic}>
                      <span className={styles.topicEmoji}>{topic.emoji}</span>
                      <span className={styles.topicName}>{topic.name}</span>
                    </td>
                    <td className={styles.tdExam}>
                      <span className={`${styles.examBadge} ${topic.exam === 'CEE' ? styles.badgeCee : styles.badgeIoe}`}>
                        {topic.exam}
                      </span>
                    </td>
                    {COLS.map((col) => (
                      <td key={col} className={styles.tdCheck}>
                        <button
                          className={`${styles.checkBtn} ${td[colKey(col)] ? styles.checkBtnOn : ''}`}
                    ...
                    <td className={styles.tdDeadline}>
                      <input
                        type="date"
                    ...
                    <td className={styles.tdDays}>
                      {topicDaysLeft !== null ? (
                        <span className={`${styles.daysChip} ${
                          ...
                          topicDaysLeft <= 3 ? styles.chipUrgent :
                          styles.chipOk
                        }`}>
                          {isDone ? '✓ Done' : topicDaysLeft < 0 ? `${Math.abs(topicDaysLeft)}d overdue` : `${topicDaysLeft}d`}
                        </span>
                      ) : (
